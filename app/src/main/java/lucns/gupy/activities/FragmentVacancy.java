...
public class FragmentVacancy extends FragmentView {
    ...
    public void onCreate() {
        ...
        vacancy.formatDates();
        if (vacancy.expirationDate != null) {
            textExpiration.setText(vacancy.expirationDate);
            if (vacancy.isExpired()) textExpiration.setTextColor(getColor(R.color.red));
        }
        if (vacancy.publicationDate != null) {
            textPublished.setText(vacancy.publicationDate);
            long passedDays = vacancy.getPassedDays(vacancy.publicationDate);
            if (passedDays >= 90) textPublished.setTextColor(getColor(R.color.red));
            else if (passedDays >= 60) textPublished.setTextColor(getColor(R.color.orange));
            else if (passedDays < 7) textPublished.setTextColor(getColor(R.color.green));
        }
